...
const ProductName = styled.span<{ status: '종료' | string }>`
  font-weight: bold;
  color: ${props => props.status === '완료' ? 'black' : props.status === '패널티' ? 'red' : 'inherit'};
`;
...
const ShippingStatus = styled.div<{ status: '배송 준비중' | '배송중' | '배송 완료' }>`
  width: 50%;
  background-color: ${props => props.status === '배송 준비중' ? '#F8D7DA' : props.status === '배송중' ? '#FFF3CD' : '#D4EDDA'};
  color: ${props => props.status === '배송 준비중' ? '#721C24' : props.status === '배송중' ? '#856404' : '#155724'};
  font-size: 15pt;
  font-weight: bold;
  display: flex;
  align-items: center;
...
interface ProductItemProps {
  ...
  announcementDate?: { daysLeft: number; status: '당첨' | '미당첨' | null } | null;
  status: '신청' | '진행중' | '당첨' | '종료';
...
const ProductItem: React.FC<ProductItemProps> = ({
  ...
  return (
    <Container>
              ...
              <>
                ...
                <ShippingContainer>
                  <ShippingStatus status={shippingStatus || '배송 준비중'}>
                    {shippingStatus === '배송중' ? '배송중' : shippingStatus === '배송 완료' ? '배송 완료' : '배송 준비중'}
                    {shippingStatus === '배송중' && <ShippingDetails>택배 송장번호: 68728181222</ShippingDetails>}
                  </ShippingStatus>
                  <ShippingInfo>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center' }}>
        {status === '신청' && (
          <>
            ...
            <StatusText>{announcementDate ? `D-${announcementDate.daysLeft}` : '미정'}</StatusText>
          </>
        )}
      ...
      {status === '종료' && <Divider />} {/* Add a divider only for '종료' status */}
    </Container>
  );
};
