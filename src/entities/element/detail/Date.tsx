...
const getBackgroundStyles = (startDate?: Date, endDate?: Date): { background: string; opacity: number } => {
  if (!startDate) {
    // 날짜가 없는 경우 
    return { background: 'linear-gradient(270deg, #f0f0f0 0%, #e0e0e0 100%)', opacity: 1 };
  }

  if (currentDate >= startDate && currentDate <= (endDate || startDate)) {
    // 현재 날짜와 겹칠 때
    return { background: 'linear-gradient(270deg, #0D1B4A 0%, #476090 50%, #7995B2 100%)', opacity: 1 };
  }
  ...
  // 현재 날짜 이전
  return { background: 'linear-gradient(270deg, #0D1B4A 0%, #476090 50%, #7995B2 100%)', opacity: 0.5 };
};
